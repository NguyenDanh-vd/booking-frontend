...
import { FileWarning, Search, ShieldAlert, SlidersHorizontal } from "lucide-react";
import { adminDataService } from "@/services/admin-data.service";
import type { IReport } from "@/types/backend";
import toast from "react-hot-toast";

type ReportType = IReport["type"] | "";
...
export default function AdminReportsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter(
        (report) =>
          (report.content || report.message || "").toLowerCase().includes(query) ||
          String(report.id).includes(query) ||
          report.sender?.fullName?.toLowerCase().includes(query)
      );
    }

    if (type) {
      data = data.filter((report) => report.type === type);
    }
  ...
  const handleResolve = (id: number) => {
    toast(`Đã xử lý báo cáo #${id} (cần bổ sung API thực tế)`);
  };
    ...
    <div className="relative mx-auto max-w-6xl space-y-6 py-8">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 p-7 shadow-xl">
        ...
        <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white">Báo cáo và khiếu nại</h1>
            <p className="mt-2 text-slate-200">
              Quản lý phản ánh, khiếu nại và các cảnh báo vi phạm trong hệ thống.
            </p>
          </div>
          <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-cyan-100">
            ...
            Tổng mục: {loading ? "..." : reports.length}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-sm md:p-6">
        <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Tìm theo nội dung, tên người gửi hoặc ID..."
              value={search}
              onChange={(e) => {
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-8 text-center">
            <p className="text-lg font-semibold text-slate-700">Không có báo cáo phù hợp</p>
                ...
                <tbody>
                  {paged.map((report) => (
                    <tr key={report.id} className="border-t border-slate-100 hover:bg-slate-50/70">
                      ...
                      <td className="px-4 py-3 text-slate-700">
                        {report.sender?.fullName || report.senderId || "-"}
                      </td>
                      <td className="px-4 py-3 text-slate-700">
...
function ReportTypeBadge({ type }: { type: IReport["type"] }) {
  if (type === "REPORT") {
    return (
